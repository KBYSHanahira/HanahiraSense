var name_text = "HanahiraSense";
var last_upd = "5/6/2021";
var version = "5";
var width = 650;
var height = 500;
...
var menu_elements = {
	"Rage": {
		"General": {
			...
			"Doubletap": [
				{"type": "checkbox", "name": "Doubletap boost", "id": "doubletap_boost"},
...
function voteRevealer(){
    ...
    Global.PrintColor([217, 217, 217, 255], "[Raphael] \0");
...
function customFog(){
    ...
    const color = UI.GetColor("Script items", "Fog color");
    const color_value = color[0] + " " + color[1] + " " + color[2];
    const distance = slider_values["fog_distance"].toString();
    const density = (slider_values["fog_density"] / 100).toString();
...
function enemyLocation(){
    ...
    for(i = 0; i < enemies.length; i++){
		if(Entity.IsAlive(enemies[i])){
			...
			var SendInfo = ("[Raphael] Player " + Name + " with " + Health + " HP " + " is at " + Location + " with weapon " + Weapon);
...
function indicators(){
	...
	if(GetValue("Desync strength indicator")){
		var size = 1.25;
		var x = screen_size[0];
    	var y = screen_size[1];
	...
	for(active_bind in active_binds){
		...
		Render.StringCustom(screen_size[0] / 2 - text_size[0] / 2, screen_size[1] / 2 + 6 + binds_offset + crosshair_size, 0, name, color, font);
...
function drawMenu(){
	...
	var text_size2 = Render.TextSizeCustom("Made Hanahira", menu_font);
	Render.StringCustom(x + width - 14 - text_size2[0], y + height - 26, 0, "Made by Hanahira", text_color, menu_font);
...
function onUnload(){
	...
	Convar.SetString("cl_lock_camera", "0");
	Cheat.ExecuteCommand("-attack2");
	Cheat.ExecuteCommand("fog_override 0");
	Convar.SetString("fog_override", "0");
	Convar.SetFloat("mat_ambient_light_r", 0);
    Convar.SetFloat("mat_ambient_light_g", 0);
    Convar.SetFloat("mat_ambient_light_b", 0);
